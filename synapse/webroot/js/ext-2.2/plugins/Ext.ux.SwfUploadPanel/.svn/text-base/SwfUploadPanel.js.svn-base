...
//	if (!config.title) config.title = 'File Upload';
	...
	this.progress_bar = new Ext.ProgressBar({
			style: 'margin-top: -1px; margin-left: -1px; margin-right:1px;margin-bottom: 1px;',
			text:'Progress Bar',
//			, width: config.width - 7
	...
	var cm = new Ext.grid.ColumnModel([{id:'name', header: "Filename", dataIndex: 'name'}
				...
				,	{id:'status', header: "Status", width: 100, dataIndex: 'status', renderer: this.formatStatus }
			]);
	...
	config = Ext.apply(config || {}, {
		...
		, bbar: new Ext.Toolbar({style:'border:0 none;overflow:hidden;padding:0px;background:#e8e8e8;position:relative;zoom:1;', items:[ this.progress_bar ]})
	...
	this.on('resize', function() { 
		this.progress_bar.setWidth(this.bbar.getWidth());	
